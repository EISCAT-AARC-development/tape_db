...
class Conn:
	...
	def check_timezone(self):
		...
		if secs:
			raise StandardError("The timezone on mysqld is set wrong, %+.1f hours"%(secs/3600.0))
...
#			if table == 'tape_comments': continue
	...
	def update_experiment(self, experiment_name, country, antenna, update_country=0):
		...
		try:
			...
			# duplicate key. Update country if not null
			update = 1
	...
	def update_resource(self, experiment_id, start, end, typestring, account, bugfix=1):
		c = self.cur
		if end is None: end = 0	# means until the next record
		c.execute("SELECT resource_id FROM resource WHERE experiment_id = %s AND start = FROM_UNIXTIME(%s) AND end = FROM_UNIXTIME(%s) AND type = %s",
				(experiment_id, start, end, typestring))
		resource_id = c.fetchone()
		resource_id = resource_id and resource_id[0]
		if typestring == 'data' and bugfix and not resource_id:
			# BUGFIX: many tapes have not counted in the integration time
			#         when calculating the start time. Compensate for that,
			#         by searching a 60 sec interval. Is there any longer
			#         integration periods?
			#   Also: Tapes have the time truncated instead of rounded to
			#         nearest second
			...
			if resource_id:
				...
				if prev_start > start:
					c.execute("UPDATE resource SET start = FROM_UNIXTIME(%s) WHERE resource_id = %s",
						(start, resource_id))
				else:
					start = prev_start
		if not resource_id:
			c.execute("INSERT INTO resource (experiment_id, start, end, type, account) VALUES (%s, FROM_UNIXTIME(%s), FROM_UNIXTIME(%s), %s,%s)",
					(experiment_id, start, end, typestring, account))
			c.execute("SELECT LAST_INSERT_ID()")
	...
	def update_location(self, resource_id, location, bytes, priority=50):
		...
		self.insert_location(resource_id=resource_id, location=location,
	...
	def update(self, experiment_name, country, antenna, start, end,
		...
		experiment_id = self.update_experiment(experiment_name,
				country=country, antenna=antenna, update_country=update_country)
		resource_id = self.update_resource(experiment_id, start, end, typestring, account)
		self.update_location(resource_id, location,
				bytes=bytes, priority=priority)
	...
	def check_urls_backwards(self, locations, keys):
		...
		c = self.cur
                # suppress SQL warnings about temporary tables, replication on deposit is set to ignore them /CFE 2016
                warnings.filterwarnings("ignore",'.*TEMPORARY.*TABLE.*')
		#c.execute("CREATE TEMPORARY TABLE temp ( string varchar(255) UNIQUE KEY )")
		#c.execute("INSERT INTO temp SELECT REVERSE(location) FROM storage")
		c.execute("CREATE TEMPORARY TABLE temp SELECT REVERSE(location) AS string FROM storage")
		#c.execute("ALTER TABLE temp ADD INDEX (string)")
		...
		for x, k in zip(locations, keys):
			c.execute(command, (x[::-1],))	# python 2.3 reverse string
			if not c.fetchone()[0]:
				res.append(k)
		# no permission to drop
	...
	def purge_broken_links(self, dry=0, backwards=0, verbose=1):
		...
		def sub(child_table, parent_table, variable, c=c, dry=dry, verbose=verbose):
			# this would be simplified in MySQL 4.1 where subqueries are
			# supported
...
#
#		cmd = "SELECT r.* FROM resource r LEFT JOIN resource AS r2 ON r.end = r2.end AND r.type = r2.type AND r.experiment_id = r2.experiment_id AND r.start < r2.start WHERE r.end = FROM_UNIXTIME(0) AND r.experiment_id = %s AND r.start <= %s GROUP BY type HAVING end = FROM_UNIXTIME(0) AND start < FROM_UNIXTIME(%s)"
...
#	def select_sources(self, middle_time, experiment_name, country, site, typestring):
#		x = self.select("experiments", experiment_name=experiment_name, site=site)
#		if not x: return []
#		experiment_id = x[0].experiment_id
#		
#		cmd = "SELECT resource_id FROM resource WHERE start < FROM_UNIXTIME(%s) AND end > FROM_UNIXTIME(%s) AND type = %s"
#		x = self.select_sql(cmd, (start, end, typestring))
...
def openMySQL_SSH(host, port=3306, interactive=0, **params):
	...
	for i in range(5):
		...
		## Converted to work with python 2.6 by MartinH 2010-06
...
def openmaster():
	if nodename() in ("data1","eiscathq"):
		return openMySQL(host="eiscathq",passwd='ks902jf4',db='disk_archive',user='archiver')
	else:
...
def opendefault():
	if nodename() in ("eiscathq"):
		...
		return openMySQL(host="eiscathq",db='disk_archive',user='www')
	else:
		raise
...
import urlparse
if 'eiscat-raid' not in urlparse.uses_netloc: urlparse.uses_netloc.append('eiscat-raid')
if 'eiscat-raid' not in urlparse.uses_query : urlparse.uses_query .append('eiscat-raid')
if 'eiscat-tape' not in urlparse.uses_netloc: urlparse.uses_netloc.append('eiscat-tape')
if 'eiscat-tape' not in urlparse.uses_query : urlparse.uses_query .append('eiscat-tape')
...
def create_raidurl(node, path, flags=()):
	...
	return urlparse.urlunsplit(("eiscat-raid", node, path, query, ''))
...
def create_tapeurl(tapenr, path):
	...
	import urlparse
	return urlparse.urlunsplit(("eiscat-tape", str(tapenr), '/'+path, '', ''))
...
def merge(localconn, remoteconn, verbose=0):
	...
	# the order here must match the Conn.update() call later
	...
	while 1:
		...
		if verbose:
			print("Source %d/%d %.2f%% ETA %d min"%(i, n, i*100.0/n,
			   (n-i)*(time.time()-t0)/i/60)	)
...
if __name__ == '__main__':
	...
	if cmd=='create':
		...
		try:
			...
			experiments = c.select("experiments",
					experiment_name=experiment_name, antenna=antenna)
	...
	#	merge(local, remote, verbose=1)
		...
		conn = openmaster()
		#n = conn.cur.execute("UPDATE storage SET bytes = 1024*bytes WHERE location LIKE 'eiscat-tape://%'")
